...
- 공개 IPv4 주소나 도메인을 `DOMAIN`으로 설정하면 Caddy가 단기 TLS 인증서를 자동 발급합니다.
- `DOMAIN=:80`은 인증서 발급 전 평문 HTTP 테스트에만 사용합니다.
...
도메인 없이 서버의 공개 IPv4 주소로 HTTPS를 사용할 때:
...
인증서 발급 전 평문 HTTP 테스트가 꼭 필요할 때만 다음 값을 사용합니다.
...
공개 IPv4 또는 도메인 모드에서는 `https://<DOMAIN>`으로 접속합니다. `DOMAIN=:80` 테스트 모드만 `http://<SERVER_IP>`를 사용합니다.
...
### 4. HTTPS로 전환하거나 도메인을 나중에 연결할 때

```bash
sudoedit /etc/wtcit.env
# DOMAIN='203.0.113.10' 또는 DOMAIN='game.example.com'으로 변경
sudo wtcit-update
```
...
평문 HTTP 테스트가 꼭 필요할 때만 다음 값을 사용합니다.
...
```bash
curl -fsS https://<SERVER_IP>/health
```
