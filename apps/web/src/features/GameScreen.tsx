...
import type { GameSettings, ParticipantRole, RoomSnapshot, SpyVoteChoice } from "@wtcit/shared";
...
interface GameScreenProps {
  ...
  onSpyHint: (hint: string) => void;
  onSpyVote: (choice: SpyVoteChoice) => void;
...
export function GameScreen(props: GameScreenProps) {
  ...
  else if (snapshot.view === "spy") content = <SpyScreen snapshot={snapshot} onHint={props.onSpyHint} onVote={props.onSpyVote} onUpdate={props.onGuessUpdate} onConfirm={props.onGuessConfirm} onPause={props.onRevealPause} onAdvance={props.onRevealAdvance} />;
  else if (snapshot.view === "precisionPlayer" || snapshot.view === "precisionSpectator") content = <PrecisionScreen snapshot={snapshot} onUpdate={props.onGuessUpdate} onConfirm={props.onGuessConfirm} />;
  ...
  else if (snapshot.phase === "gameOver") content = <GameOverScreen snapshot={snapshot} onBack={props.onEnd} />;
  else content = null;
