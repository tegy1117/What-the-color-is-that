...
export function RevealScreen({ snapshot, onPause, onAdvance }: { snapshot: RevealSnapshot; onPause: (paused: boolean) => void; onAdvance: () => void }) {
  ...
  return <main className={`${styles.page} ${styles.revealPage}`}><div className={styles.revealHeading}><span>{t("reveal.title")}</span><h1>{snapshot.reveal.hint}</h1></div><div className={styles.revealLayout}><section className={styles.comparison}><div className={styles.comparisonSwatches}><ColorResult label={t("reveal.target")} color={snapshot.reveal.targetHex} />{comparison ? <ColorResult label={selfResult ? t("reveal.mine") : t("reveal.closest")} color={comparison.color} /> : null}</div>{comparison ? <><div className={styles.difference}><span>{t("reveal.difference")}</span><strong>ΔE {comparison.deltaE.toFixed(1)}</strong>{comparison.autoSubmitted ? <small>{t("reveal.autoSubmitted")}</small> : null}</div><div className={styles.scoreBreakdown}><span>{t("reveal.accuracy", { value: comparison.accuracy })}</span><span>{t("reveal.speed", { value: comparison.speedPoints })}</span><strong>{t("reveal.roundScore", { value: comparison.roundScore })}</strong></div></> : null}<div className={styles.pickerScore}>{t("reveal.pickerScore", { value: snapshot.reveal.pickerScore })}<small>{snapshot.reveal.pickerNickname}</small></div><div className={styles.allResults}>{snapshot.reveal.results.map((result) => <div key={result.participantId}><i style={{ backgroundColor: result.color }} /><span>{result.nickname}</span><b>{result.roundScore}</b></div>)}</div></section><aside className={styles.resultRail}><Ranking ranking={snapshot.ranking} title={t("reveal.ranking")} /><div className={styles.revealControls}><Countdown deadline={snapshot.deadline} serverNow={snapshot.serverNow} totalSeconds={12} paused={snapshot.revealPaused} remainingMs={snapshot.revealRemainingMs} /><p>{snapshot.revealPaused ? t("reveal.paused") : t("reveal.nextIn", { seconds: Math.ceil(((snapshot.deadline ?? snapshot.serverNow) - snapshot.serverNow) / 1000) })}</p>{isHost ? <><button className={styles.secondaryButton} type="button" onClick={() => onPause(!snapshot.revealPaused)}>{snapshot.revealPaused ? <PlayIcon /> : <PauseIcon />}{snapshot.revealPaused ? t("reveal.resume") : t("reveal.pause")}</button><button className={styles.darkButton} type="button" onClick={onAdvance}>{t("reveal.next")}<ArrowIcon /></button></> : null}</div></aside></div></main>;
}
